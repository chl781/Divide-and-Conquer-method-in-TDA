...
boundFind <- function(ind_suspicious,Diag_split,X_split,m,gap1,gap2,eps){
  bound = matrix(list(),m-1,m-1) # This is to save all the boundary points with respect to every features.
  ...
  for (i in 1:(m-1)) {
    for (j in 1:(m-1)) {
      ...
      bound[[i,j]] = matrix(list(),1,length(ind_suspicious_ij))
      Suspicious[[i,j]] = matrix(list(),1,length(ind_suspicious_ij))
      Suspicious_features[[i,j]] = matrix(list(),1,length(ind_suspicious_ij))
      # This discussion is for general.
      lengthX = NROW(X_split[[i,j]])
      
      for(ij in 1:length(ind_suspicious_ij)){
        bound1 = c() # This indicates the boundary points
        bound2 = c() # This indicates which boundaries are included.
        data1 = Diag_split_ij[["cycleLocation"]][[ ind_suspicious_ij[ij] ]]
        IN = as.vector(data1) # Show the indices for ind_suspicious_ij[ij]
        IN1 = IN-lengthX 
        IN1 = IN1[IN1>0] # Only leave the indices of boundaries.
        IN2 = unique(IN1)
        
        #gap_space = (gap3[2]-gap3[1])/lengthX*5
        lengthBound = max( floor(lengthX/5) + 2, floor((gap1[2] - gap1[1])/eps) )  # lengthBound determine the size of supplemental points on boundaries.
        ...
        data2 = X_split_ij[IN[IN<=lengthX],] # Representative data for the feature.
       
        
        # Will introduce another parameter about the gap between points.
        if(1 %in% IN1){  
          if(i!=1){
            bound1 = rbind(bound1,cbind(rep(gap3[1], lengthBound), 
        ...
        if(sum(ind)==0){ # insignificant case.
